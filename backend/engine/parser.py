...
def _get_executor() -> LlmExecutor:
    ...
    """
    provider = _os.environ.get("LLM_PROVIDER", "sdk").strip().lower()
    if provider not in ("sdk", "cli"):
        logger.warning("未知 LLM_PROVIDER=%s，回退为 sdk", provider)
        provider = "sdk"

    sdk_type = _os.environ.get("LLM_SDK_TYPE", "anthropic").strip().lower()
...
def _extract_json(text: str, schema_type: str = "intent") -> dict:
    ...
    改进的 fence 检测：支持 ```json 后跟任意文本、行尾空格等情况
    ...
    # 提取 ```json ... ``` 块 — 使用更稳健的正则匹配
    # 兼容 ```json 后跟额外文本、行尾空格、Windows 换行符等情况
    json_fence_re = _re.compile(r"```(?:json)?\s*\n?", _re.MULTILINE)
    m = json_fence_re.search(text)
    if m:
        start = m.end()  # 跳过 fence 标记本身
