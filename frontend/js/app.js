...
(function () {
  ...
  // ---- 安全的 HTML 转义（纯字符串替换，避免 DOM 创建开销） ----
  var _escapeMap = {
    "&": "&amp;",
    "<": "&lt;",
    ">": "&gt;",
  ...
  function _buildPlanHtml(data) {
    var planId = "plan-" + Date.now();
    return '<div style="margin-top:8px;padding:10px;background:#f0fdf4;border:1px solid #22c55e;border-radius:6px;font-size:12px">'
      + '<strong>计划:</strong><br><pre style="margin:4px 0;white-space:pre-wrap;font-family:inherit">' + escapeHtml(data.plan) + '</pre>'
      ...
      + '<input type="text" id="' + planId + '" placeholder="或者说明怎么改..." style="flex:1;font-size:12px;padding:4px 8px;border:1px solid #d1d5db;border-radius:4px">'
      + '<button style="font-size:12px;padding:4px 8px;background:#3b82f6;color:#fff;border:none;border-radius:4px;cursor:pointer" onclick="app.cancelPlan();var v=document.getElementById(\'' + planId + '\').value.trim();if(v){app.submitProgress(v)}">修改</button>'
      + '</div>'
      + '</div>';
  }

  function _buildAskHtml(data) {
    var askId = "ask-" + Date.now();
    var html = '<div style="margin-top:8px;padding:10px;background:#fffbeb;border:1px solid #f59e0b;border-radius:6px;font-size:13px">'
      + '<strong>AI:</strong><br>' + escapeHtml(data.question);
    if (data.options && data.options.length > 0) {
      html += '<div style="margin-top:6px">'
        + data.options.map(function(o) {
            return '<button style="margin:2px;font-size:11px;padding:3px 8px;cursor:pointer" onclick="app.submitProgressFollowup(\'' + escapeHtml(o).replace(/'/g,"\\'") + '\')">' + escapeHtml(o) + '</button>';
          }).join("")
        + '</div>';
    }
    html += '<div style="margin-top:8px;display:flex;gap:4px">'
      ...
      + '<button style="font-size:12px;padding:4px 8px;background:#3b82f6;color:#fff;border:none;border-radius:4px;cursor:pointer;white-space:nowrap" onclick="var v=document.getElementById(\'' + askId + '\').value.trim();if(v)app.submitProgressFollowup(v)">发送</button>'
      + '</div>'
      + '</div>';
    return html;
  }
  ...
  async function _handleProgressResponse(data, inputEl) {
    ...
    if (data.action === "confirm_plan") {
      ...
      var planHtml = _buildPlanHtml(data);
      var psEl = document.getElementById("progress-status");
      if (psEl) { psEl.innerHTML = planHtml; psEl.className = "status-msg success"; }
      return;
    }
    // AI 反问
    if (data.action === "ask") {
      var askHtml = _buildAskHtml(data);
      var psEl = document.getElementById("progress-status");
      if (psEl) { psEl.innerHTML = askHtml; psEl.className = "status-msg warning"; }
      return;
    }
    // 成功
  ...
  async function addNodeNL(optFollowUp) {
    ...
    // 构建请求体 — 移除 _lastAddNodeDesc 隐式确认机制，改为显式 _pendingOps 确认
    ...
    try {
      ...
      if (data.action === "ask") {
        var askHtml = _buildAskHtml(data);
        var asEl = document.getElementById("add-node-status");
        if (asEl) { asEl.innerHTML = askHtml; asEl.className = "status-msg warning"; }
        return;
      }
